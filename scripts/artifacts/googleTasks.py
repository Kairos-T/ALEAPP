...
def protobuf_parse_not_completed(data):
    ...
    created = datetime.fromtimestamp(pb[0].get('11',{}).get('1','')).strftime('%Y-%m-%d %H:%M:%S')
    modified = datetime.fromtimestamp(pb[0].get('3',{}).get('1','')).strftime('%Y-%m-%d %H:%M:%S')
...
def protobuf_parse_completed(data):
    ...
    task = pb[0].get('2',{}).get('2','').decode()
    task_details = b2s(pb[0].get('2',{}).get('3',''))
    completed = datetime.fromtimestamp(pb[0].get('2',{}).get('5',{}).get('1','')).strftime('%Y-%m-%d %H:%M:%S')
    created = datetime.fromtimestamp(pb[0].get('11',{}).get('1','')).strftime('%Y-%m-%d %H:%M:%S')
    modified = datetime.fromtimestamp(pb[0].get('3',{}).get('1','')).strftime('%Y-%m-%d %H:%M:%S')
    # duetime = pb[0].get('9',{}).get('1',{}).get('6',{}).get('1','')
    # duetime = datetime.fromtimestamp(duetime).strftime('%Y-%m-%d %H:%M:%S') # TypeError: an integer is required (got type str)
